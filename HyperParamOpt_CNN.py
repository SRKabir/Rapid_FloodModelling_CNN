...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, Flatten, Dense, LSTM, TimeDistributed, BatchNormalization, Activation
...
def create_model(x_train, y_train, x_test, y_test, steps, features, outputs):
    ...
    model.add(Conv1D({{choice([16, 32, 64, 128, 256, 512])}}, kernel_size=1, activation='relu', input_shape=(steps, features)))

    model.add(Conv1D({{choice([16, 32, 64, 128, 256, 512])}}, kernel_size=1, activation='relu'))
